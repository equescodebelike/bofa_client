    ...
    return Scaffold(
      ...
      body: widget.isEditing
          ...
          : BlocListener<ProductListBloc, ProductListState>(
              listener: (context, state) {
                if (state is ProductListOperationSuccess) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(state.message)),
                  );
                  Navigator.of(context).pop();
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (widget.isEditing) {
        context
            ...
            .add(UpdateProductDetail(widget.product!.productId!, product));
      } else {
        context.read<ProductListBloc>().add(CreateProductInList(product));
      }
    }
  }
}
